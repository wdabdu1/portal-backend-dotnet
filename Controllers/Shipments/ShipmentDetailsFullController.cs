...
public record ShipmentLineItemDetail(string ProductCategory, string ProductType, string ModelProduct, decimal QtyInBl, string? UnitOfMeasure, string? HsCode, decimal? UnitPrice, string? Currency, decimal? Total);
...
public record ShipmentFullDetailResponse(
    ...
    string BusinessUnit, string? Division, string? Supplier, string Consignee, string Category, string Type,
    string? ShippingLine, string? VesselName, int Fcl20Count, int Fcl40Count, DateOnly? Etd, DateOnly? Eta, DateOnly? SobActualDate,
...
public class ShipmentDetailsFullController : ControllerBase
{
    ...
    public async Task<ActionResult<ShipmentFullDetailResponse>> Get(int id, [FromServices] BuAccessService buAccess)
    {
        ...
        var lineItems = shipment.LineItems.Select(li => new ShipmentLineItemDetail(
            ...
            li.PurchaseOrderLineItem?.ProductType?.Name ?? "",
        ...
        var type = lineItems.FirstOrDefault()?.ProductType ?? "";
        ...
        return new ShipmentFullDetailResponse(
            ...
            shipment.PurchaseOrder.Consignee?.Name ?? "", category, type, shipment.ShippingLine?.Name, shipment.VesselName,
